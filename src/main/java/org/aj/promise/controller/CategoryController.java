...
public class CategoryController {
  ...
  public Response<String> addCategory(@RequestParam("name") String name, Author user) {
    if (StringUtils.isAlphanumeric(name)) {
      ...
      Category category = Category.builder().authorId(user.getId()).name(name).count(0).build();
  ...
  public Response<String> deleteCategory(@RequestParam("id") String id, Author user) {
    if (StringUtils.isAlphanumeric(id)) {
      ...
      if (category != null && category.getCount() == 0 && user.getId().equals(category.getAuthorId())) {
