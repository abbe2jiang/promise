...
public class MvcConfig implements WebMvcConfigurer {
  ...
  public void addResourceHandlers(ResourceHandlerRegistry registry) {
    ...
    for (String item : dirs) {
      ResourceHandlerRegistration registration = registry.addResourceHandler("/" + item + "**")
          .addResourceLocations("file:static/" + item);
      if (!commonProperties.isDebug()) {
        registration.setCacheControl(CacheControl.maxAge(30, TimeUnit.DAYS));
      }
    }
    // image
    ResourceHandlerRegistration registration = registry.addResourceHandler("/image/**")
        .addResourceLocations(getImageLocation() + "/image/").setCacheControl(CacheControl.maxAge(7, TimeUnit.DAYS));
    if (!commonProperties.isDebug()) {
      registration.setCacheControl(CacheControl.maxAge(7, TimeUnit.DAYS));
    }
