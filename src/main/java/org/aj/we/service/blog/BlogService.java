...
public class BlogService {
  @Autowired BlogMongoRepository blogMongoRepository;

  @Autowired CommentMongoRepository commentMongoRepository;

  @Autowired private MongoTemplate template;

  public Blog add(Blog blog) { return blogMongoRepository.save(blog); }
  ...
  public Pagination<Blog> getBlogsByAuthorId(Pageable pageable,
                                             String authorId) {
  ...
  public Pagination<Blog> getBlogsByCategoryId(Pageable pageable,
                                               String categoryId) {
  ...
  public Blog getBlog(String id) { return blogMongoRepository.findAllById(id); }

  public void updateComments(String blogId) {
    long count = commentMongoRepository.countByBlogId(blogId);
    template.updateFirst(Query.query(Criteria.where("id").is(blogId)),
                         Update.update("comments", count), Blog.class);
  }

  public void updateCategory(String sourceCategoryId, String targetCategoryId) {
    template.updateMulti(
        Query.query(Criteria.where("categoryId").is(sourceCategoryId)),
